...
)
{
    ...
    ReloaderBlock->PAEEnable = IsPAEEnable();
        ...
        while (0 != _CmpByte(ControlPc[0], 0xc2) &&
            ...
            if (5 == Length) {
                if (0 == _CmpByte(ControlPc[0], 0xa1)) {
                    ReloaderBlock->PfnDatabase = *(PVOID*)*(PULONG)(ControlPc + Length - sizeof(LONG));
                ...
                while (0 != _CmpByte(ControlPc[0], 0xc2) &&
                    ...
                    if ((5 == Length && 0 == _CmpByte(ControlPc[0], 0xa1)) ||
                        ...
                        if (NULL == ReloaderBlock->FreePageSlist[0]) {
                            ...
                            ReloaderBlock->FreePageSlist[0] = *(PVOID *)TargetPc;
                        }
                        else if (NULL == ReloaderBlock->FreePageSlist[1]) {
                            TargetPc = *(PVOID *)(ControlPc + Length - sizeof(LONG));

                            ReloaderBlock->FreePageSlist[1] = *(PVOID *)TargetPc;
...
{
    return FALSE != ReloaderBlock->PAEEnable ?
        (PMMPTE)(((((ULONG)VirtualAddress) >> 21) << 3) + (ULONG)ReloaderBlock->PdeBase) :
        (PMMPTE)(((((ULONG)VirtualAddress) >> 22) << 2) + (ULONG)ReloaderBlock->PdeBase);
}
...
{
    return FALSE != ReloaderBlock->PAEEnable ?
        (PMMPTE)(((((ULONG)VirtualAddress) >> 12) << 3) + (ULONG)ReloaderBlock->PteBase) :
        (PMMPTE)(((((ULONG)VirtualAddress) >> 12) << 2) + (ULONG)ReloaderBlock->PteBase);
}
...
{
    return FALSE != ReloaderBlock->PAEEnable ?
        (PVOID)((ULONG)Pte << 9) : (PVOID)((ULONG)Pte << 10);
}
...
{
    return FALSE != ReloaderBlock->PAEEnable ?
        (PVOID)((ULONG)Pde << 18) : (PVOID)((ULONG)Pde << 20);
}
...
)
{
    ...
    if (ReloaderBlock->BuildNumber < 9200) {
        if (FALSE != ReloaderBlock->PAEEnable) {
            Pfn = &ReloaderBlock->PfnDatabase->Pfn7600Ex + PageFrameIndex;
        }
        else {
...
)
{
    ...
    if (ReloaderBlock->BuildNumber < 9200) {
        if (FALSE != ReloaderBlock->PAEEnable) {
            PageFrameIndex = &Pfn->Pfn7600Ex - &ReloaderBlock->PfnDatabase->Pfn7600Ex;
        }
        else {
...
)
{
    ...
    if (0 != ReloaderBlock->FreeSlist.Depth) {
        TempPfn = InterlockedPopEntrySList(&ReloaderBlock->FreeSlist);
        ...
        for (SlistIndex = 0;
            ...
            for (Color = 0;
                ...
                if ((MAXIMUM_PFNSLIST_DEPTH - DepthIndex) ==
                    ...
                    TempPfn = InterlockedPopEntrySList(
...
)
{
    ...
    InterlockedPushEntrySList(&ReloaderBlock->FreeSlist, Next);
}
