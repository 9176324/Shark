...
NTSTATUS ClassReadDriveCapacity(IN PDEVICE_OBJECT Fdo)
{
    ...
    //
    driveCapMdl = BuildDeviceInputMdl(&readCapacityData, sizeof(READ_CAPACITY_DATA_EX));
    if (driveCapMdl == NULL) {
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto SafeExit;
    }
...
RetryRequest:
    ...
    if (NT_SUCCESS(status) &&
       (pseudoIrp.IoStatus.Information < readCapacityDataSize)) {
        ...
        DBGERR(("ClassReadDriveCapacity: read len (%xh) < %xh, retrying ...",
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (match == FALSE) {
            ...
            ClasspUpdateDiskProperties(Fdo);
        }
    ...
    case IOCTL_STORAGE_READ_CAPACITY: {
        ...
        if (!commonExtension->IsFdo) {
            ...
            if (NT_SUCCESS(status) && fdoData->IsCachedDriveCapDataValid) {
                ...
                //
                DebugPrint((1, "ClassDeviceControl: ClassReadDriveCapacity failed: 0x%X IsCachedDriveCapDataValid: %d\n",
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        DebugPrint((ClassDebugWarning, "ClasspUpdateDiskProperties: Disk property update for fdo %p failed with status 0x%X.\n", Fdo, status));
    } else {
        DebugPrint((ClassDebugTrace, "ClasspUpdateDiskProperties: Drive capacity has changed for %p.\n", Fdo));
    }
