...
//  video preview pin. If called the first time, it creates an analog
//  video preview pin object and stores the analog video preview pin
//  pointer in the pin context. If not called the first time, it retrieves
//  the video preview out pin pointer from the pin context.
//  It also calls the setting of the data format on the instance
//  of the analog video preview pin.
//
// Return Value:
//  STATUS_UNSUCCESSFUL  Operation failed,
//                       (a) if the function parameters are zero,
//                       (b) if the analog video preview pin pointer is zero
//                       (c) SetDataFormat failed
//  STATUS_INSUFFICIENT_RESOURCES
//                       Operation failed,
//                       if the analog video preview pin could't be created
//  STATUS_INVALID_DEVICE_STATE
//                       SetDataFormat on an video preview pin failed,
//                       if the Pin DeviceState is not KSSTATE_STOP
//  STATUS_SUCCESS       SetDataFormat on an video preview pin with success
//
//////////////////////////////////////////////////////////////////////////////
 ...
 )
{
    ...
    _DbgPrintF(DEBUGLVL_BLAB,("Info: AnlgVideoPrevPinSetDataFormat called"));
    ...
    Framing -> FramingItem [0].Frames = NUM_VD_PREV_STREAM_BUFFER;
...
//  The AnlgVideoPrevIntersectDataFormat is the IntersectHandler function
//  of the analog video preview out pin.(Needs no class environment to intersect
//  the format, because the format that is supported is already
//  given in the filter descriptor [ANLG_VIDEO_PREV_PIN_DESCRIPTOR]).
// Settings:
...
//  STATUS_SUCCESS          AnlgVideoPrevIntersectDataFormat with success
...
{
    _DbgPrintF(DEBUGLVL_BLAB,(""));
    _DbgPrintF(DEBUGLVL_BLAB,("Info: AnlgVideoPrevIntersectDataFormat called"));
 ...
 )
{
    ...
    _DbgPrintF(DEBUGLVL_BLAB,(""));
    _DbgPrintF(DEBUGLVL_BLAB,("Info: AnlgVideoPrevPinCreate called"));
    //  parameters valid?
    if( !pKSPin || !pIrp )
    {
        _DbgPrintF(DEBUGLVL_ERROR,("Error: Invalid argument"));
    ...
    CAnlgVideoPrev* pAnlgVideoPrevPin = new (NON_PAGED_POOL) CAnlgVideoPrev();
    if( !pAnlgVideoPrevPin )
    {
        _DbgPrintF(DEBUGLVL_ERROR,
    ...
    pKSPin->Context = pAnlgVideoPrevPin;
    //call class function
    Status = pAnlgVideoPrevPin->Create(pKSPin, pIrp);
    if( Status != STATUS_SUCCESS )
    {
        _DbgPrintF( DEBUGLVL_ERROR,
            ("Error: Class function called without success"));
        delete pAnlgVideoPrevPin;
        pAnlgVideoPrevPin = NULL;
        pKSPin->Context = NULL;
        return Status;
    }
    return STATUS_SUCCESS;
